...
bool FEncodeSimpleFunctionParametersTest::RunTest(const FString& Parameters)
{
	...
	const FString FunctionName = "balanceOf";
	...
	const FString Encoded = USequenceSupport::EncodeFunctionCall(FunctionSignature, FunctionName, JsonValues);

	AddInfo(FString::Printf(TEXT("Simple Encoding Test: FunctionSignature: %s, FunctionName: %s, Values: %s"), *FunctionSignature, *FunctionName, *JsonValues));
