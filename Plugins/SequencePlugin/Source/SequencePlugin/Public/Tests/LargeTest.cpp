...
int64 GetBalance(FAddress From, FAddress To)
{
	...
	return HexStringToUint64(ContractBalance.GetValue().ToHex()).GetValue();
}
...
bool LargeTest::RunTest(const FString& Parameters)
{
	...
	TResult<FNonUniformData> TransactionHash1 = MakeValue(FNonUniformData::Empty());
	auto ContractAddress1 = provider.DeployContract(SmallByteCode, PrivateKey1, 1337, TransactionHash1);
	auto TransactionReceipt1 = provider.TransactionReceipt(FHash256::From(TransactionHash1.GetValue().Arr)).GetValue();
	...
	TResult<FNonUniformData> TransactionHash2 = MakeValue(FNonUniformData::Empty());
	auto ContractAddress2 = provider.DeployContract(LongByteCode, PrivateKey1, 1337, TransactionHash2);
	auto TransactionReceipt2 = provider.TransactionReceipt(FHash256::From(TransactionHash2.GetValue().Arr)).GetValue();
	...
	TResult<FNonUniformData> TransactionHash3 = MakeValue(FNonUniformData::Empty());
	auto ContractAddress3 = provider.DeployContract(VMByteCode, PrivateKey1, 1337, TransactionHash3);
	auto TransactionReceipt3 = provider.TransactionReceipt(FHash256::From(TransactionHash3.GetValue().Arr)).GetValue();
	...
	//deploy PrivateKey2
	TResult<FNonUniformData> TransactionHash4 = MakeValue(FNonUniformData::Empty());
	auto ContractAddress4 = provider.DeployContract(VMByteCode, PrivateKey2, 1337, TransactionHash4);
	auto TransactionReceipt4 = provider.TransactionReceipt(FHash256::From(TransactionHash4.GetValue().Arr)).GetValue();
	...
	if(HexStringToUint64(ContractBalance.GetValue().ToHex()) != 100) return false;
	...
	auto contractNonce = provider.TransactionCount(Address2, EBlockTag::Latest).GetValue();
	auto contractGasPrice= provider.getGasPrice().GetValue(); 
	...
	auto contractCallGas = provider.estimateContractCallGas(mContractCall).GetValue();
	...
	contractData = ABI::Encode("refill(uint256)", Properties2);
	 ...
	 contractNonce = provider.TransactionCount(Address2, EBlockTag::Latest).GetValue();
	 contractGasPrice= provider.getGasPrice().GetValue(); 
	...
	contractCallGas = provider.estimateContractCallGas(mContractCall2).GetValue();
