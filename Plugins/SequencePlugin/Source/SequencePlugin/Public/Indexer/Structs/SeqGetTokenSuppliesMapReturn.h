...
struct SEQUENCEPLUGIN_API FSeqGetTokenSuppliesMapReturn
{
		...
		if (json_in.TryGetObjectField(TEXT("supplies"), data))//safely get
		{
			...
			{//now for each map key we can add it in with our data!
				...
				if (data_map.Find(i)->Get()->TryGetArray(lst))
				{
					...
					if (FJsonObjectConverter::JsonObjectToUStruct<FSeqTokenSupplyList>(json_step.ToSharedRef(), &w_in))
					{
						supplies.Add(TPair<FString, FSeqTokenSupplyList>(i, w_in));//add the newly formed structure with it's key!
					}
				}
			}
		}
