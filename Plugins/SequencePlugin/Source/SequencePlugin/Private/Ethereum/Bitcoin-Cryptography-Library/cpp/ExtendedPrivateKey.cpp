...
ExtendedPrivateKey ExtendedPrivateKey::getChildKey(uint32_t _index) const {
	uint8_t msg[37];
	if (_index < HARDEN)  // Normal child key
		publicKey.toCompressedPoint(msg);
	else {  // Hardened child key
		msg[0] = 0;
		privateKey.getBigEndianBytes(&msg[1]);
	}
	Utils::storeBigUint32(_index, &msg[33]);
	...
	return ExtendedPrivateKey(num, &hash[32], static_cast<uint8_t>(depth + 1), _index, pubKeyHash);
}
