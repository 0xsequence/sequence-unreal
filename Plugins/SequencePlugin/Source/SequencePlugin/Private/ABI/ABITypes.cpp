...
FABIFixedBytesProperty::FABIFixedBytesProperty() : TABIPropertyWithValue<FNonUniformData>(FNonUniformData::Empty())
{
	
}

FABIFixedBytesProperty::FABIFixedBytesProperty(FNonUniformData InitialValue) : TABIPropertyWithValue<FNonUniformData>(InitialValue)
{
	
}

FABIArg FABIFixedBytesProperty::Serialize()
{
	const auto Data = new uint8[GBlockByteLength + value.Length];
	...
	Data[GBlockByteLength - 4] = (length & 0xFF000000) >> 24;
	Data[GBlockByteLength - 3] = (length & 0x00FF0000) >> 16;
	Data[GBlockByteLength - 2] = (length & 0x0000FF00) >> 8;
	Data[GBlockByteLength - 1] = (length & 0x000000FF) >> 0;

	for(uint32 i = 0; i < value.Length; i++)
	{
		Data[i + GBlockByteLength] = value.Arr[i];
	}

	return FABIArg{
		STATIC, GBlockByteLength + value.Length, Data
	};
}

FABIArg FABIFixedBytesProperty::BlankArg()
{
	return FABIArg{
		STATIC, GBlockByteLength + value.Length, nullptr
	};
}

void FABIFixedBytesProperty::Deserialize(FABIArg Arg)
{
	auto Data = static_cast<uint8*>(Arg.Data);
	auto Length = (0xFF000000 & (Data[GBlockByteLength - 4] << 24)) |
		(0x00FF0000 & (Data[GBlockByteLength - 3] << 16)) |
		(0x0000FF00 & (Data[GBlockByteLength - 2] << 8)) |
		(0x000000FF & (Data[GBlockByteLength - 1] << 0));

	auto Result = FNonUniformData {
		new uint8[Length], Length
	};

	for(auto i = 0; i < Length; i++)
	{
		Result.Arr[i] = Data[GBlockByteLength + i];
	}
